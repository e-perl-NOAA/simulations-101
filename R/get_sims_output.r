...
run_plot_compars <- function(model_names,
  ...
  plotList <- lapply(
    em_names,
    function(i, spawn_bio_new){
      spawn_bio_i <- spawn_bio_new |>
        dplyr::filter(Model %in% c(i, "om")) |>
        dplyr::mutate(Model = gsub("_.*","", Model))
      
      x <- ggplot2::ggplot(spawn_bio_i, aes(Yr, median, colour = Model, linetype = Model)) +
        ...
        labs(title = paste0("Recdev_", gsub("\\D","", i)), y = "SpawnBio", x = "Year") +
        theme_classic()
      x
      }, spawn_bio_new
  )
