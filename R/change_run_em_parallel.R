...
base_change_run_em_parallel <- function(model_dir,
  ...
  furrr::future_map(df$em_names, ~ em_change_run(em_names = .x, model_dir = model_dir, 
                                                 df = df, exe_filepath = exe_filepath, convergence.df = convergence.df))
  
  utils::write.csv(convergence.df, paste0(dir_root_name,"/model_convergence.csv"))
}

em_change_run <- function(em_names, 
                          model_dir, 
                          df, 
                          exe_filepath,
                          convergence.df){
  em_iterations <- list.dirs(file.path(model_dir, paste0("em_", em_names)), 
                             full.names = TRUE, recursive = FALSE)
  df_new <- df[df$em_names == em_names,]
  for(i in 1:length(em_iterations)){
    ...
    inputs[[df_new$config_file]][[df_new$var_change]]<- df_new$new_val
