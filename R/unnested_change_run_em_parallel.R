...
run_iters <- function(model_dirs,
                      ...
                      exe_filepath){
  start_time <- Sys.time()
  clean_plots(model_dirs[1])
  dir_iter_number <- stringr::str_sub(model_dirs[1], start= -2)
  em_iter_number <- gsub(".*em_|-iter.*","", model_dirs[1])
  df <- df[df$em_names == em_iter_number,]
  inputs <- r4ss::SS_read(dir = model_dirs[1])
  inputs[[df$config_file]][[df$var_change]]<- df$new_val
  inputs$start$N_bootstraps <- 2
  
  r4ss::SS_write(inputs, dir = model_dirs[1], overwrite = TRUE)
  
  # run initial model
  r4ss::run(dir = model_dirs[1], exe = exe_filepath, skipfinished = FALSE)
  ...
  ctl_file_in <- file.path(model_dirs[1], inputs$start$ctlfile)
  ...
  if(file.exists(file.path(model_dirs[1],"covar.sso")) & any(grep("do not write", readLines(file.path(dir_iter,"covar.sso")))) == FALSE){
    ...
    if(dir_iter_number %in% c("_1", "_2")){
      ...
      file.rename(file.path(model_dirs[1], "plots"), file.path(model_dirs[1], "plots_no_bias_adjust"))
      file.rename(file.path(model_dirs[1], "Report.sso"), file.path(model_dirs[1], "Report_no_bias_adjust.sso"))
      file.rename(file.path(model_dirs[1], "ss_summary.sso"), file.path(model_dirs[1], "ss_summary_no_bias_adjust.sso"))
    }
    
    # re-run model
    r4ss::run(dir = model_dir[1], exe = exe_filepath, verbose = FALSE, skipfinished = FALSE)
  ...
  convergence <- data.frame(
    model = gsub(paste0(".*", new_filename,"/|-em_.*"),"",model_dirs[1]),
    em = df$em_names,
    iteration = basename(model_dirs[1]),
    model_converged <- ifelse(file.exists(file.path(model_dirs[1], "covar.sso")) & 
                                any(grepl("do not write", readLines(file.path(model_dirs[1], "covar.sso")))) == FALSE, TRUE, FALSE),
    model_runtime <- runtime
  )
  
  # remove unnecessary files
  admb_files_delete <- list.files(path = model_dir[1], pattern = "*.std|*.htp|*.eva|*.tds|*.bar|*.cov|*.dep|*.hes|*.rpt|*.rep|*.cpp|*.log|*.obj|*.tmp|*.ecm|*.mc2|*.mcm|*.hst|*.psv|gradient.*|tmp_admb|variance|SIS_table.sso|rebuild.sso|posterior_obj_func.sso|posterior_vectors.sso|posteriors.sso|derived_posteriors.sso|CumReport.sso|ParmTrace.sso|runnumber.ss$|ss.p0*|ss.b0*|ss.r0*",
                                  full.names = TRUE)
