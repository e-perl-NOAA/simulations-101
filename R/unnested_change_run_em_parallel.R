...
run_iters <- function(model_dir,
  ...
  if(file.exists(file.path(model_dir, "Report.sso"))){
    ...
    if(file.exists(file.path(model_dir,"covar.sso")) & any(grep("do not write", readLines(file.path(model_dir,"covar.sso")))) == FALSE)
      {
      bias <- tryCatch(r4ss::SS_fitbiasramp(replist,
                                   # plotdir = dir_bias,
                                   plot = FALSE, 
                                   print = FALSE,
                                   oldctl = ctl_file_in,
      ...
      if(exists("message", where = bias) == FALSE){
    ...
    convergence <- data.frame(
      ...
      model_runtime = "MODEL DID NOT RUN"
    )
    return(convergence)
  }
}
