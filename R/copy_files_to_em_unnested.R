...
copy_files_to_em_unnested <- function(dir, 
                             ...
                             new_filename,
                             exe_filepath){
  dirs <- list.dirs(dir(dir, recursive = FALSE, pattern = "models$", full.names = TRUE), recursive = FALSE)
  ...
  for(m in 1:length(dirs)){
    ...
    # run(dir = dirs[m], exe = exe_filepath, extras = "-nohess -stopph 0", skipfinished = FALSE)
    inputs <- r4ss::SS_read(dir = dirs[m])
    
    starter_files <- paste0("control.ss_new|forecast.ss_new|starter.ss_new|wtatage.ss_new")
    ...
    for(o in 1:length(om_folders)){
      ...
      for(e in 1:length(df$em_names)){
        ...
        copy_data_boot <-list.files(om_folders[o], pattern = "data_boot_[0-9]", full.names = TRUE)
        ...
        file.rename(file.path(iter_folder,"control.ss"), file.path(iter_folder, inputs$start$ctlfile))
      }
    }
  }
}
